...
In the Australian Football League (AFL), 18 teams compete in the **regular** season across 24 rounds. - Teams that win a match receive 4 points - Teams that draw receive 2 points - Teams that lose receive 0 points
...
# Keep it simple here (but you'd probably want to derive difference between metrics)
rf_fit <- 
  rf_spec |> 
...
grid <- grid |> 
  mutate(team = case_when(
    ...
    opp = case_when(
      opp == 'Sydney' ~ 'Sydney',
      opp == 'GWS' ~ 'GWS',
      opp == 'Brisbane Lions' ~ 'Brisbane Lions',
      opp == 'Carlton' ~ 'Carlton',
      opp == 'Western Bulldogs' ~ 'Footscray',
      opp == 'Hawthorn' ~ 'Hawthorn',
      opp == 'Port Adelaide' ~ 'Port Adelaide',
      opp == 'Geelong' ~ 'Geelong'))
